...
This image just shows how the k8s workflow looks in realtime


![alt text](images/k8s.png)

This image is to understand how kubectl, eksctl and docker works.
...
This image also show that an init container and the main container will use the same storage space. init container runs first and brings the necessary passwords or secrets from the AWS secret manager and places it in the storage and it gets terminated. Now, the main container comes up and uses the secrets fetched by the init container and gets the necessary details from the DB as per the user request.

As per the requirment it will push the logs of the main container in to an external logstack like EKS or Dynatrace. When the pod gets deleted the logs also gets deleted, so we are pushing the logs to a Logstack

#### inorder to create k8s resources for roboshop project
    please use the below images from nagashankar1992332

        nagashankar1992332/cart
        nagashankar1992332/catalogue
        nagashankar1992332/mysql
        nagashankar1992332/shipping
        nagashankar1992332/payment
        nagashankar1992332/user
        nagashankar1992332/frontend
        nagashankar1992332/mongodb
...
#### 3) Multi-container Pod	
    ...
    Sometimes we get the same error when there is much time in getting the image from our image hub, in that case we have to directly delete the pod and then re-create a new one.
...
####   12) Service   13) service-np.yaml   and   14)service-lb.yaml
    ...
    apiVersion: v1   # this is a pod definition file
    kind: Pod
    metadata:
        name: nginx
        labels:
    ...
    apiVersion: v1    # this is a pod definition file for NodePort service
    kind: Pod
    metadata:
        name: nginx-np
        labels:
    ...
    apiVersion: v1     # this is a pod definition file for loadbalancer service
    kind: Pod
    metadata:
        name: nginx-lb
        labels:
...
####    15) replicase
    ...
    Pod is the subset of replicaset
...
####   16) deployment 
    ...
    metadata:
        ...
        labels:    # deployment labels
            ...
            app: nginx
    spec:
        replicas: 3  # These are the labels replica set use to create pod replicas, this should match pod labels
        selector:
            matchLabels:
                purpose: deployment-demo
                project: roboshop
    ...
    Understand that pod is a subset of replicaset and replicaset is a subset of Deployment

    So, if we create a deployment, k8s will create a deployment, replicaset and a pod.
